...
#' @param alpha Numeric in (0, 1). The quantile level used to compute the threshold via \code{\link{Fit_F}}. Default is 0.01.
...
plot_RD <- function(data_list,
  ...
  p <- ggplot(df_RD_HR, aes(x = distance)) +
    ...
    (if (show_f_density) geom_line(aes(x = x, y = y), size = 0.8, color = '#333333') else NULL) +
