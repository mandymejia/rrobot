...
#' @param alpha Numeric in (0, 1). The quantile level used to compute the HR threshold via \code{\link{Fit_F}} if \code{HR} is not provided. Default is 0.01.
#' @param HR Optional numeric vector of threshold(s) for robust distances. If \code{NULL} (default), the threshold is computed via \code{\link{Fit_F}} for each dataset. If a single value is provided, it will be applied to all datasets. If a named vector is provided, names must match dataset names.
  ...
  for (i in seq_along(datanames)) {
    ...
    FPs_i <- replicate(n_rep, {
      ...
      thresh <- if (is.null(HR)) {
        Fit_F(Q, t, rob_tmp$h, quantile = alpha)$threshold
      } else if (length(HR) == 1) {
        HR
      } else {
        HR[dist_name]
