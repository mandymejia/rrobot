...
#' Overlays vertical threshold lines (computed via \code{\link{Fit_F}}) and a scaled F-distribution curve.
...
#' @param alpha Numeric in (0, 1). The quantile level used to compute the HR threshold via \code{\link{Fit_F}}. Default is 0.01.
...
#' }
#'
#' @seealso \code{\link{comp_RD}}, \code{\link{Fit_F}}
#' @importFrom ggplot2 ggplot aes geom_histogram geom_line geom_vline
#'   scale_fill_manual scale_color_manual theme_minimal
...
plot_RD <- function(data_list,
  ...
  for (i in seq_along(datanames)) {
    ...
    df_HR_label <- rbind(
      ...
      data.frame(
        x = q_HR,
        y = max(yvals, na.rm = TRUE) * 1.05,  # Slightly above the curve
  ...
  ggplot(df_RD_HR, aes(x = distance)) +
    ...
    geom_line(aes(x = x, y = y), size = 0.8, color = '#333333') +
    facet_layer +
